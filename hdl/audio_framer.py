...
CHAN_FIELD = 8
SEQ_FIELD = 24
assert SAMPLE_FIELD + CHAN_FIELD + SEQ_FIELD == WORD_W
...
class AudioFramer(Elaboratable):
    ...
    def __init__(self, *, n_channels: int, sample_w: int, fifo_depth: int = 256):
        assert 1 <= sample_w <= SAMPLE_FIELD
    ...
    @staticmethod
    def pack(seq: int, chan: int, sample: int) -> int:
        s32 = sample & (2 ** SAMPLE_FIELD - 1)            # two's complement, 32b
        return ((seq & (2 ** SEQ_FIELD - 1)) << (SAMPLE_FIELD + CHAN_FIELD)
                | (chan & (2 ** CHAN_FIELD - 1)) << SAMPLE_FIELD
                | s32)

    @staticmethod
    def unpack(word: int):
        sample = word & (2 ** SAMPLE_FIELD - 1)
        if sample >= 2 ** (SAMPLE_FIELD - 1):
            sample -= 2 ** SAMPLE_FIELD                    # sign-extend
        chan = (word >> SAMPLE_FIELD) & (2 ** CHAN_FIELD - 1)
        seq = (word >> (SAMPLE_FIELD + CHAN_FIELD)) & (2 ** SEQ_FIELD - 1)
        return seq, chan, sample
    ...
    def elaborate(self, platform):
        ...
        sample_field = Signal(SAMPLE_FIELD)
        m.d.comb += [
            chan_field.eq(self.in_chan),
            sample_field.eq(self.in_sample),              # sign-extends to 32b
            fifo.w_data.eq(Cat(sample_field, chan_field, seq[self.in_chan])),
